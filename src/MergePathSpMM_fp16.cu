...
int main(int argc, char *argv[]) {
    ...
    /* Weight Matrix */
    __half *h_input  = (__half *) malloc(NODE_ACT_NUM * dimension * sizeof(__half));
    __half *h_output = (__half *) malloc(NODE_ACT_NUM * dimension * sizeof(__half)); 
    ...
    cudaMalloc((void**) &d_input, NODE_ACT_NUM * dimension * sizeof(__half));
    cudaMemcpy(d_input, h_input, NODE_ACT_NUM * dimension * sizeof(__half), cudaMemcpyHostToDevice);
    ...
    cudaMalloc((void**) &d_output, NODE_ACT_NUM * dimension * sizeof(__half));
    cudaMemset(&d_output, 0, NODE_ACT_NUM * dimension* sizeof(__half)); 
    ...
    if (dimension <= WARP_SIZE) {
        ...
        for (int i = 0; i < repeats; i++) {
            spmm_merge_path<<<grid, BLOCK>>>(
                (__half *) d_output, (__half *) d_input, 
    ...
    cudaMemcpy(h_output, d_output, NODE_ACT_NUM * dimension * sizeof(__half), cudaMemcpyDeviceToHost);

    // for (int i = 0; i < NODE_NUM; i++) {
    //     for (int j = 0; j < dimension; j++) {
    //         cout <<  __half2float(h_output[i * dimension + j]) << "-";
    //     }
    //     std::cout << endl;
    // }
...
__global__ void spmm_merge_path(
    ...
    if (warpId < num_warps) {
        ...
        __half partial_results_start = 0;
        __half  partial_results_end = 0;
        __half output_temp = 0; 
        __half degree_norm_inv = 0;
        __half src_norm = 0;
        int index = 0;
        int num_features = 0;
        ...
        if (fstart != 0) {
            ...
            for (int j = 0; j < fstart_num; j++) {
                ...
                degree_norm_inv =  __hmul(src_norm, degrees[index]);
                partial_results_start +=  __hmul(degree_norm_inv, input[index * dim + laneid]); 
        ...
        for (int i = start; i < end; i++) {
            ...
            for (int j = 0; j < num_features; j++) {
                ...
                degree_norm_inv =  __hmul(src_norm, degrees[index]);
                output_temp += __hmul(degree_norm_inv, input[index * dim + laneid]);
                features_start++;
            }
        ...
        if (fend != 0) {
            ...
            for (int j = 0; j < fend_num; j++) {
                ...
                degree_norm_inv =  __hmul(src_norm, degrees[index]);
                partial_results_end += __hmul(degree_norm_inv, input[index * dim + laneid]); 
            } 
